...
<body>
    ...
    <div class="container-fluid ">
        ...
        <div class="row left-component" style="height: 100%">
            ...
            <div class="col-md-10">
                ...
                <div style="text-align: center;line-height: 200%;height: 50px;font-size: 30px;">存款</div>
                    ...
                    <div  class="col-md-12 " >
                        <form>
                            <fieldset>
                                ...
                                <p>
                                    <label for="amount">存入金额:</label>
                                    <input type="text" id="amount" name="amount" align="left">
                                </p>
                                <p>
                                    <button  class="btn btn-info " id="depositbtn" type="button">存入</button>
                                    <span id="depositmsg" style="font-size: 35px;line-height: 150%"></span>
                                </p>
                            </fieldset>
                        </form>
                    </div>
    ...
    <script>
        $(function (){
            $("#depositbtn").attr("cardid","${cardid}");

            $("#depositbtn").click(function (){
                var amount = $("#amount").val();
                var pattern = /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]{1,2}$)/;
                if (!pattern.test(amount)){
                    $("#depositmsg").text("存款金额输入不合法,请重新输入");
                    return false;
                }
                if (!confirm("确认客户${customername}存入" + $("#amount").val() + "元")){
                    return false;
                }
                $.ajax({
                    url:"${APP_PATH}/deposit/" + $(this).attr("cardid"),
                    type:"PUT",
                    // data:$("form").serialize(),
                    data:"amount=" + amount + "&customername=" + "${customername}",
                    success:function (data){
                      $("#depositmsg").text(data.extend.msg);
                    }
                });
            });
        })
