...
<body>
    ...
    <div class="container-fluid ">
        ...
        <div class="row left-component" style="height: 100%">
            ...
            <div class="col-md-10">
                ...
                <div style="text-align: center;line-height: 200%;height: 50px;font-size: 30px;">取款</div>
                    ...
                    <div  class="col-md-12 " >
                        <form>
                            <fieldset>
                                ...
                                <p>
                                    <button  class="btn btn-info " id="withdrawbtn" type="button">取款</button>
    ...
    <script>
        $(function (){
            $("#withdrawbtn").attr("cardid","${cardid}");
            $("#withdrawbtn").attr("balance","${balance}");
            $("#withdrawbtn").click(function (){
                var amount = $("#amount").val();
                var pattern = /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]{1,2}$)/;
                if (!pattern.test(amount)){
                    $("#msg").text("取款金额输入不合法,请重新输入");
                    return false;
                }

                if (parseInt(amount)>parseInt($("#withdrawbtn").attr("balance"))){
                    $("#msg").text("余额不足");
                    return false;
                }
                $(this).attr("balance",parseFloat( $("#withdrawbtn").attr("balance")) - parseFloat(amount));

                if (!confirm("确认客户${customername}取款" + $("#amount").val() + "元")){
                    return false;
                }
                $.ajax({
                    url:"${APP_PATH}/withdraw/" + $(this).attr("cardid"),
                    type:"PUT",
                    data:$("form").serialize() + "&customername=${customername}",
